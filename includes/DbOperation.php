...
class DbOperation
{
	...
	function getHeroes()
	{
		...
		while($stmt->fetch())
		{
			...
			echo " <br> ". " ID: ". $hero['id']. " <br> NOME: ".$hero['name']." <br> NOME REAL: ".$hero['realname']." <br> NOTA: ".$hero['rating']." <br> AFILIAÇÃO: ".$hero['teamaffiliation']. " <br> " ; 
	...
	function getHeroesEsp($id)
	{
		$stmt = $this->con->prepare("SELECT * FROM heroes WHERE id = ?");
		$stmt->bind_param("i", $id);
		$stmt->bind_result($id, $name, $realname, $rating, $teamaffiliation);

		$stmt->execute();
		
		echo " <br> ". " ID: ". $id. " <br> NOME: ".$name." <br> NOME REAL: ".$realname." <br> NOTA: ".$rating." <br> AFILIAÇÃO: ".$teamaffiliation. " <br> " ; 
